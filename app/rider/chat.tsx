...
export default function ChatScreen() {
  ...
  const otherPerson = chat?.driver;
  const otherPersonName = getPersonName(otherPerson, 'Driver');
  ...
  const initializeChat = async () => {
    try {
      ...
      if (!chatData?.id && rideId) {
        ...
        try {
          const createResponse = await apiService.createChat(rideId as string);
  ...
  const renderMessage = ({ item }: { item: Message }) => (
    <ChatMessage
      ...
      onLocationShare={(locationData) => {
        // Live location update - broadcast to driver
  ...
  const renderMessageSkeleton = () => (
    <>
      <MessageSkeleton isOwn={false} backgroundColor={colors.surface} />
      <MessageSkeleton isOwn={true} backgroundColor={colors.primary} />
      <MessageSkeleton isOwn={false} backgroundColor={colors.surface} />
      <MessageSkeleton isOwn={true} backgroundColor={colors.primary} />
    </>
  );
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { color: colors.text, marginTop: verticalScale(12) }]}>
            Loading chat...
          </Text>
        </View>
      </View>
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.errorText, { color: colors.text, marginTop: verticalScale(12) }]}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            ...
            <View style={styles.emptyContainer}>
              <MaterialCommunityIcons name="message" size={48} color={colors.textTertiary} />
